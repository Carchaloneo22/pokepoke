...
(function(global) {
  ...
  class TileMapRenderer {
    constructor(containerId, options = {}) {
      ...
      this.flowerPatchImage.src = 'assets/sprites/flower_patch.png';

      // Sprite oficial de Brendan (256x256 con celdas 64x64)
      this.brendanSprite = new Image();
      this.brendanSprite.src = 'assets/sprites/brendan_spritesheet.png';
    ...
    updateCamera() {
      if (!this.canvas) return;
      const dpr = window.devicePixelRatio || 1;
      const viewWLog = this.canvas.width / (dpr * this.fixedZoom);
      const viewHLog = this.canvas.height / (dpr * this.fixedZoom);
